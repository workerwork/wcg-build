...
do
    ...
    enbinfo_logs=$(ls /root/eGW/Logs/Record/.enbInfo.log* 2>/dev/null)
    ...
    for logs in $egw_logs $sctpd_logs $vtysh_logs $enbinfo_logs $egw_report_logs $egw_manage_logs $egw_monitor_logs
    do
