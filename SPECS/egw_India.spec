...
#BuildRequires:
AutoReqProv: no
Requires:   nginx,redis,hiredis,fcgi,spawn-fcgi,gsoap,curl,lksctp-tools,vconfig,keepalived,net-tools,ethtool,wget,sysstat,pciutils    
...
sed -i 's@\(/etc/\)\(eGW\)@\1.\2@' startAll.sh
...
for file in startAll.sh stopAll.sh OMC/startOm.sh OMC/stopOm.sh
do
...
cp -rf TR069/eGW_OMCLogCfg.conf $RPM_BUILD_ROOT/etc/.eGW
...
%exclude /usr/lib/eGW/TR069/eGW_OMCLogCfg.conf
...
function coredump_set() {
    ...
    echo "kernel.core_pattern = $core_path/coredump/core-%e-sig%s-user%u-group%g-pid%p-time%t" > /etc/sysctl.d/core.conf
